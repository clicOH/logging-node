...
- `LOG_HTTP_FULL`: includes sanitized request headers in HTTP and audit logs. Request bodies are logged by the service after redacting sensitive fields.
- `LOG_HTTP_MASK_PATH`: when `true`, masks path IDs, UUIDs, emails, and tokens as `:value`. It defaults to `false`, so `url` contains the real path without its query string. `route` always contains the masked path for grouping in Loki.
